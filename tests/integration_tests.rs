...
async fn test_pubsub_context_nack() {
    ...
    assert!(
        ...
        "Message should have been negatively acknowledged"
    );
}
...
async fn test_pubsub_context_ack_error() {
    ...
    assert!(
        result.is_err(),
        "Ack should fail when the underlying function fails"
    );
}
...
async fn test_pubsub_context_nack_error() {
    ...
    assert!(
        result.is_err(),
        "Nack should fail when the underlying function fails"
    );
}
...
fn test_pubsub_context_clone() {
    ...
    assert_eq!(
        ctx1.ack_id, ctx2.ack_id,
        "Cloned context should have the same ack_id"
    );
}
...
fn test_pubsub_context_debug() {
    ...
    assert!(
        debug_str.contains("PubSubContext"),
        "Debug output should contain struct name"
    );
    assert!(
        debug_str.contains("test-debug-id"),
        "Debug output should contain ack_id"
    );
}
